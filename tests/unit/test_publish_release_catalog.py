...
def _release_zip(path: Path, version: str = "0.7.2") -> None:
...
def test_validate_archive_requires_matching_manifest_and_updater(tmp_path):
    ...
    validate_archive(archive, "0.7.2")
...
def test_validate_archive_rejects_root_level_updater(tmp_path):
    ...
    with zipfile.ZipFile(archive, "a") as zip_file:
        zip_file.writestr("MonStim_Analyzer_v0.7.2-WIN/MonStim Updater.exe", b"duplicate updater")
...
def test_validate_archive_requires_internal_license(tmp_path):
    ...
    root = "MonStim_Analyzer_v0.7.2-WIN/"
    with zipfile.ZipFile(archive, "w") as zip_file:
        ...
        zip_file.writestr(root + "_internal/monstim-release.json", json.dumps({"version": "0.7.2", "executable": "MonStim Analyzer.exe"}))
...
def test_update_catalog_replaces_matching_release_and_sorts_versions():
    catalog = {
        ...
        "releases": [
            ...
            {"version": "0.7.2", "channel": "stable", "platform": "windows-x86_64"},
        ],
    }
    entry = {"version": "0.7.2", "channel": "beta", "platform": "windows-x86_64", "sha256": "a" * 64}

    updated = update_catalog(catalog, entry)

    assert next(item for item in updated["releases"] if item["channel"] == "beta" and item["version"] == "0.7.2") == entry
    assert [item["version"] for item in updated["releases"]].count("0.7.2") == 2
    assert next(item for item in updated["releases"] if item["channel"] == "beta" and item["version"] == "0.7.2") == entry
