...
| Displayed post-stimulus time | 1000.0 ms | Default time shown after the stimulus in plots; shorter recordings remain limited to their acquired width. |
| Displayed pre-stimulus time | 250.0 ms | Default baseline shown before the stimulus; shorter recordings remain limited to their acquired width. |
...
| Plateau variation threshold | 0.15 | How much the M-wave values may vary within a plateau, relative to the high-response signal level. `0.15` allows up to 15% variation. |
...
The M-wave recognition names are global user settings, not profile settings. Edit them in **File > Settings Center > Global Analysis > Latency windows**. The M-max estimator also uses a third-order smoothing polynomial, a 95th-percentile candidate, and a top-20% candidate. It reports M-max as unavailable when no plateau is detected. These are reproducibility settings, not physiological constants. See [M-max estimation](mmax_estimation.md) before using M-max normalization.
