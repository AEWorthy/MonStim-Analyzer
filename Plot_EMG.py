...
class EMGSessionPlotter(EMGPlotter):
    ...
    def plot_emg (self, channel_names=[], m_flags = False, h_flags = False, data_type='filtered'):
        ...
        if data_type == 'filtered':
            if not hasattr(self.session, 'recordings_processed'):
                self.session.recordings_processed = self.session.process_emg_data(apply_filter=True, rectify=False)
    ...
    def plot_emg_suspectedH (self, channel_names=[], h_threshold=0.3, plot_legend=False):
        ...
        """
        # Ensure that the EMGSession has processed recordings.
        if not hasattr(self.session, 'recordings_processed'):
            self.session.recordings_processed = self.session.process_emg_data(apply_filter=True, rectify=False)
    ...
    def plot_reflex_curves (self, channel_names=[], method='rms', relative_to_mmax=False, manual_mmax=None):
        ...
        for channel_index in range(self.session.num_channels):
            ...
            for recording in self.session.recordings_processed:
                ...
                if method == 'rms':
                    m_wave_amplitude = emg_transform.calculate_rms_amplitude(channel_data, self.session.m_start[channel_index] + self.session.stim_delay, self.session.m_end[channel_index] + self.session.stim_delay, self.session.scan_rate)
                    h_response_amplitude = emg_transform.calculate_rms_amplitude(channel_data, self.session.h_start[channel_index] + self.session.stim_delay, self.session.h_end[channel_index] + self.session.stim_delay, self.session.scan_rate)
                elif method == 'avg_rectified':
                    m_wave_amplitude = emg_transform.calculate_average_amplitude_rectified(channel_data, self.session.m_start[channel_index] + self.session.stim_delay, self.session.m_end[channel_index] + self.session.stim_delay, self.session.scan_rate)
                    h_response_amplitude = emg_transform.calculate_average_amplitude_rectified(channel_data, self.session.h_start[channel_index] + self.session.stim_delay, self.session.h_end[channel_index] + self.session.stim_delay, self.session.scan_rate)
                elif method == 'peak_to_trough':
                    m_wave_amplitude = emg_transform.calculate_peak_to_trough_amplitude(channel_data, self.session.m_start[channel_index] + self.session.stim_delay, self.session.m_end[channel_index] + self.session.stim_delay, self.session.scan_rate)
                    h_response_amplitude = emg_transform.calculate_peak_to_trough_amplitude(channel_data, self.session.h_start[channel_index] + self.session.stim_delay, self.session.h_end[channel_index] + self.session.stim_delay, self.session.scan_rate)
                else:
            ...
            if relative_to_mmax:
                if manual_mmax is not None:
                    ...
                    m_max = emg_transform.get_avg_mmax(stimulus_voltages, m_wave_amplitudes, **self.session.m_max_args)
        ...
        for channel_index in range(self.dataset.num_channels):
            ...
            for stimulus_v in stimulus_voltages:
                ...
                for recording in recordings:
                    ...
                    if binned_stimulus_v == stimulus_v:
                        channel_data = recording['channel_data'][channel_index]
                        if method == 'rms':
                            m_wave_amplitude = emg_transform.calculate_rms_amplitude(channel_data, self.dataset.m_start[channel_index] + self.dataset.stim_delay, self.dataset.m_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                            h_response_amplitude = emg_transform.calculate_rms_amplitude(channel_data, self.dataset.h_start[channel_index] + self.dataset.stim_delay, self.dataset.h_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                        elif method == 'avg_rectified':
                            m_wave_amplitude = emg_transform.calculate_average_amplitude_rectified(channel_data, self.dataset.m_start[channel_index] + self.dataset.stim_delay, self.dataset.m_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                            h_response_amplitude = emg_transform.calculate_average_amplitude_rectified(channel_data, self.dataset.h_start[channel_index] + self.dataset.stim_delay, self.dataset.h_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                        elif method == 'peak_to_trough':
                            m_wave_amplitude = emg_transform.calculate_peak_to_trough_amplitude(channel_data, self.dataset.m_start[channel_index] + self.dataset.stim_delay, self.dataset.m_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                            h_response_amplitude = emg_transform.calculate_peak_to_trough_amplitude(channel_data, self.dataset.h_start[channel_index] + self.dataset.stim_delay, self.dataset.h_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                        else:
                            print(f">! Error: method {method} is not supported. Please use 'rms', 'avg_rectified', or 'peak_to_trough'.")
                            return
            ...
            if relative_to_mmax:
                if manual_mmax is not None:
                    ...
                    m_max = emg_transform.get_avg_mmax(stimulus_voltages, m_wave_means, **self.dataset.m_max_args)
        ...
        for channel_index in range(self.dataset.num_channels):
            ...
            for recording in recordings:
                ...
                if binned_stimulus_v == max_h_reflex_voltage:
                    channel_data = recording['channel_data'][channel_index]
                    if method == 'rms':
                        m_wave_amplitude_max_h = emg_transform.calculate_rms_amplitude(channel_data, self.dataset.m_start[channel_index] + self.dataset.stim_delay, self.dataset.m_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                        h_response_amplitude_max_h = emg_transform.calculate_rms_amplitude(channel_data, self.dataset.h_start[channel_index] + self.dataset.stim_delay, self.dataset.h_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                    elif method == 'avg_rectified':
                        m_wave_amplitude_max_h = emg_transform.calculate_average_amplitude_rectified(channel_data, self.dataset.m_start[channel_index] + self.dataset.stim_delay, self.dataset.m_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                        h_response_amplitude_max_h = emg_transform.calculate_average_amplitude_rectified(channel_data, self.dataset.h_start[channel_index] + self.dataset.stim_delay, self.dataset.h_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                    elif method == 'peak_to_trough':
                        m_wave_amplitude_max_h = emg_transform.calculate_peak_to_trough_amplitude(channel_data, self.dataset.m_start[channel_index] + self.dataset.stim_delay, self.dataset.m_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                        h_response_amplitude_max_h = emg_transform.calculate_peak_to_trough_amplitude(channel_data, self.dataset.h_start[channel_index] + self.dataset.stim_delay, self.dataset.h_end[channel_index] + self.dataset.stim_delay, self.dataset.scan_rate)
                    else:
                        print(f">! Error: method {method} is not supported. Please use 'rms', 'avg_rectified', or 'peak_to_trough'.")
                        return
