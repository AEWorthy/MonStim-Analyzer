"""
Update annot JSON files with date_added and date_modified fields.
...
def process_file(path: Path, now_iso: str) -> bool:
    ...
    if is_older:
        ...
        # Update date_added if missing
        if "date_added" not in data or not data.get("date_added"):
            data["date_added"] = now_iso
            changed = True

        # Always update date_modified to now
