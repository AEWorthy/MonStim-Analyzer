...
class PlotController:
    ...
    def initialize(self):
        ...
        logger.debug("PlotController initialized successfully")
    ...
    def _disable_plot_controls(self):
        ...
        try:
            ...
            logger.warning(f"Could not disable plot controls: {e}")
    ...
    def _enable_plot_controls(self):
        ...
        try:
            ...
            logger.warning(f"Could not re-enable plot controls: {e}")
    ...
    def plot_data(self, return_raw_data: bool = False):
        ...
        self._plot_count += 1
        logger.debug(f"=== Starting plot operation #{self._plot_count} ===")
        ...
        try:
            ...
            logger.error(f"Plot configuration error: {e}")
            return None
        ...
        if level is None:
            ...
            logger.warning("No level selected for plotting data.")
            return None
        ...
        for hook in self._pre_plot_hooks:
            try:
                ...
                logger.warning(f"Error in pre-plot hook: {hook_error}")
        ...
        try:
            ...
            logger.debug("Plot rendering completed successfully.")
            ...
            try:
                if hasattr(self.gui.plot_pane, "graphics_layout"):
                    ...
                    logger.debug("Graphics layout update() called to stabilize scene")
            except Exception as e:
                logger.warning(f"Could not call update() on graphics layout: {e}")
        ...
        logger.debug(f"Plot Created. level: {level} type: {plot_type}, return_raw_data: {return_raw_data}.")
        ...
        if self._block_input_during_graphics_update:
            ...
            logger.debug("User input blocked during graphics update phase")
        ...
        def safe_layout_update():
            ...
            try:
                logger.debug("Attempting deferred layout update...")
                if hasattr(self.gui.plot_pane, "layout") and self.gui.plot_pane.layout:
                    self.gui.plot_pane.layout.update()
                    logger.debug("Layout update completed successfully.")
                else:
                    logger.warning("Plot pane layout not available for update")
            except (AttributeError, RuntimeError) as e:
                logger.warning(f"Could not update plot pane layout: {e}")
            except Exception as e:
                logger.exception(f"CRITICAL: Unexpected error during layout update: {e}", exc_info=True)
            finally:
        ...
        try:
            ...
            logger.debug("Layout update scheduled successfully")
        except Exception as e:
            logger.exception(f"CRITICAL: Failed to schedule layout update: {e}", exc_info=True)
        ...
        logger.debug(f"Executing {len(self._post_plot_hooks)} post-plot hooks...")
        for i, hook in enumerate(self._post_plot_hooks):
            try:
                logger.debug(f"Executing post-plot hook {i + 1}/{len(self._post_plot_hooks)}...")
                hook(plot_result)
                logger.debug(f"Post-plot hook {i + 1} completed successfully.")
            except Exception as hook_error:
                logger.warning(f"Error in post-plot hook {i + 1}: {hook_error}", exc_info=True)
        ...
        if self._enable_auto_gc:
            logger.debug("Scheduling deferred garbage collection (500ms delay)...")

            def deferred_gc():
                """Safely perform garbage collection after Qt events are fully processed."""
                try:
                    logger.debug("Starting deferred garbage collection...")
                    collected = gc.collect()
                    logger.debug(f"Garbage collection completed. Collected {collected} objects.")
                    logger.debug(f"Session totals: {self._plot_count} plots created so far")
                except Exception as gc_error:
                    logger.error(f"CRITICAL: Deferred garbage collection failed: {gc_error}", exc_info=True)

            # Schedule GC for 500ms later - gives Qt plenty of time to finish all operations
            try:
                QTimer.singleShot(500, deferred_gc)
                logger.debug("Deferred GC scheduled successfully")
            except Exception as e:
                logger.error(f"CRITICAL: Failed to schedule deferred GC: {e}", exc_info=True)
        else:
            logger.debug("Automatic garbage collection disabled (potential stability issue).")

        logger.debug("Plot operation completed successfully. Returning control to GUI.")
        ...
        def heartbeat_check():
            try:
                logger.debug(f"Heartbeat: Plot #{self._plot_count} - GUI event loop responsive after plot completion")

                # Re-enable user input after graphics operations are complete
                if self._is_in_graphics_critical_section:
                    self._enable_plot_controls()
                    self._is_in_graphics_critical_section = False
                    logger.debug("User input re-enabled after graphics update completion")
            except Exception as e:
                ...
                try:
                    ...
                    logger.error("Failed to re-enable plot controls in heartbeat error handling", exc_info=True)

        try:
            # Re-enable input after ~50ms (after layout update and GC are complete)
            QTimer.singleShot(50, heartbeat_check)
        except Exception as e:
            logger.error(f"CRITICAL: Failed to schedule heartbeat check: {e}", exc_info=True)
            # Emergency: re-enable controls immediately if scheduling fails
            self._enable_plot_controls()
            self._is_in_graphics_critical_section = False

        if return_raw_data:
            logger.debug(f"Returning raw data with {len(raw_data) if raw_data is not None else 0} entries.")
            return raw_data

        logger.debug("Returning None from plot_data()")
        return None
    ...
    def get_raw_data(self):
        ...
        try:
            ...
            logger.error(f"Configuration error in get_raw_data: {e}")
            return
    ...
    def handle_unable_to_plot_error(self, e, plot_type, plot_options):
        ...
        logger.exception(
            f"Unable to plot: {error_msg}"
            f"Plot type: {plot_type}, options: {plot_options}"
    ...
    def handle_plot_error(self, e, plot_type, plot_options):
        ...
        logger.error(error_msg)
        logger.error(f"Plot type: {plot_type}, options: {plot_options}")
