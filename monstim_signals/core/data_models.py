    ...
    def __post_init__(self):
        ...
        if self.stim_duration is None:
            self.stim_duration = (
                ...
                self.stim_delay
                + ((self.pulse_period + self.ramp_duration * 2 + self.peak_duration) * self.num_pulses - (self.pulse_period + self.ramp_duration))
            )
...
class RecordingAnnot:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> RecordingAnnot:
        ...
        for invalid_key in raw.keys() - valid:
            logger.warning(f"Invalid key '{invalid_key}' found in RecordingAnnot dict. Ignoring it.")
...
class SessionAnnot:
    """
    SessionAnnot is a data class that encapsulates all user edits and annotations for a session. It tracks which recordings are excluded, custom
    latency windows, optional cached m_max values, and custom channel names (if the user renamed channels). The class provides methods to create
    empty annotations, initialize from recording metadata, and construct from a dictionary (e.g., loaded from JSON), handling extra or unexpected
    keys gracefully.
    Attributes:
    ...
    Methods:
        ...
        from_dict(cls, raw: dict[str, Any]) -> 'SessionAnnot':
            Builds a SessionAnnot from a dictionary (such as one loaded from JSON), filtering out unexpected keys
            and converting nested structures as needed.
    """
    ...
    def from_dict(cls, raw: dict[str, Any]) -> SessionAnnot:
        ...
        filtered = {k: v for k, v in raw.items() if k in valid}
        for invalid_key in raw.keys() - valid:
            logger.warning(f"Invalid key '{invalid_key}' found in SessionAnnot dict. Ignoring it.")
...
class DatasetAnnot:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> DatasetAnnot:
        ...
        filtered = {k: v for k, v in raw.items() if k in valid}
        for invalid_key in raw.keys() - valid:
            logger.warning(f"Invalid key '{invalid_key}' found in DatasetAnnot dict. Ignoring it.")
...
class ExperimentAnnot:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> ExperimentAnnot:
        ...
        filtered = {k: v for k, v in raw.items() if k in valid}
        for invalid_key in raw.keys() - valid:
            logger.warning(f"Invalid key '{invalid_key}' found in ExperimentAnnot dict. Ignoring it.")
