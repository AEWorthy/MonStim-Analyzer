...
class ExperimentPlotterPyQtGraph(BasePlotterPyQtGraph):
    ...
    def _plot_reflex_curves_data(self, plot_item, channel_idx, method, relative_to_mmax, manual_mmax):
        ...
        try:
            ...
            if 'M-wave' in reflex_data.columns:
                ...
                if not m_data.empty:
                    plot_item.plot(stimulus_voltages, m_data.values, 
            ...
            if 'H-reflex' in reflex_data.columns:
                ...
                if not h_data.empty:
                    plot_item.plot(stimulus_voltages, h_data.values,
    ...
    def plot_maxH(self, channel_indices: List[int] = None, method=None, relative_to_mmax=False, 
        ...
        try:
            ...
            for i, channel_idx in enumerate(channel_indices):
                ...
                plot_item = canvas.addPlot(row=row, col=col)
                plot_item.setLabel('left', f'Channel {channel_idx + 1}')
                plot_item.setLabel('bottom', 'Dataset')
                plot_item.showGrid(True, True)
    ...
    def _plot_max_h_data(self, plot_item, channel_idx, method, relative_to_mmax, 
        ...
        try:
            ...
            # Create dataset labels for x-axis
            dataset_labels = [f"Dataset {i+1}" for i in range(len(max_h_data))]
            ...
            # Set x-axis labels
            plot_item.getAxis('bottom').setTicks([list(zip(x_positions, dataset_labels))])
    ...
    def _plot_mmax_data(self, plot_item, channel_idx, method):
        ...
        try:
            ...
            # Create dataset labels for x-axis
            dataset_labels = [f"Dataset {i+1}" for i in range(len(mmax_data))]
            x_positions = np.arange(len(dataset_labels))
