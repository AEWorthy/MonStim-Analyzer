...
class SessionPlotter(BasePlotter):
    ...
    def plot_channel_data(
        ...
        line = ax.plot(
            ...
            color=color,
            label=f"Stimulus Voltage: {stimulus_v}",
        )
    ...
    def plot_latency_windows(self, ax, all_flags, channel_index):
        if all_flags:
            for window in self.emg_object.latency_windows:
                ...
                if not start_exists:
                    ax.axvline(
                        window.start_times[channel_index],
                        color=window.color,
                ...
                if not end_exists:
                    ax.axvline(
                        window.end_times[channel_index],
                        color=window.color,
                        linestyle=window.linestyle,
                    )

    def set_fig_labels_and_legends(
        self,
        fig,
    ...
    def plot_emg(
        ...
        norm = plt.Normalize(
            vmin=min(self.emg_object.stimulus_voltages),
            vmax=max(self.emg_object.stimulus_voltages),
        )
    ...
    def plot_emg_thresholded(
        self,
        latency_window_oi_name: str,
        channel_indices: List[int] = None,
        emg_threshold_v: float = 0.3,
        method: str = None,
    ...
    def plot_suspectedH(
        ...
        if not has_h_reflex_latency_window:
            raise UnableToPlotError(
                "No H-reflex latency window detected. Please add an H-reflex latency window to the session."
            )
        self.plot_emg_thresholded(
            "H-reflex",
            channel_indices=channel_indices,
            emg_threshold_v=h_threshold,
            method=method,
            all_flags=all_flags,
    ...
    def plot_mmax(
        ...
        for channel_index in range(self.emg_object.num_channels):
            ...
            m_max, mmax_low_stim, _ = transform.get_avg_mmax(
                stimulus_voltages,
                m_wave_amplitudes,
                return_mmax_stim_range=True,
                **self.emg_object.m_max_args,
            )
            ...
            if num_channels == 1:
                ...
                ax.annotate(
                    ...
                    xy=(m_x + 0.2, np.mean(m_max_amplitudes) - 0.2),
                    ha="left",
                    va="center",
                    color="black",
                )
                ax.errorbar(
                    m_x,
                    np.mean(m_max_amplitudes),
                    yerr=np.std(m_max_amplitudes),
                    color="black",
                    marker="+",
                    markersize=10,
                    capsize=10,
                ...
                axes[channel_index].annotate(
                    f"n={len(m_max_amplitudes)}\nM-max: {m_max:.2f}mV\nStim.: above {mmax_low_stim:.2f}V",
                    xy=(m_x + 0.2, np.mean(m_max_amplitudes)),
                    ha="left",
                    va="center",
                    color="black",
                )
    ...
    def plot_m_curves_smoothened(
        ...
        for channel_index in range(self.emg_object.num_channels):
            ...
            try:
                ...
                logging.warning(
                    f"Failed to retrieve EMG amplitudes for channel {channel_index} in session {self.emg_object.id}."
                )
                continue
